...
/** \file
 * \brief
 * EtherCAT AF_XDP socket driver.
 *
 * AF_XDP backend for the SOEM Linux NIC driver. It exposes exactly the
 * same ecx_* interface as the AF_PACKET backend (nicdrv_pkt.c), but the
 * data path goes through an AF_XDP (XSK) socket with a UMEM shared
 * memory region, bypassing most of the kernel network stack.
 *
 * All AF_XDP state is held per-port inside ecx_portt::xsk (and the
 * redundant port inside ecx_redportt::xsk), so there are no global
 * singletons. The indexed RX buffer / WKC matching / redundancy
 * decision tree logic is identical to the AF_PACKET backend.
 *
 * The XSK file descriptor is stored into port->sockhandle so that the
 * existing ppoll()-based wait path keeps working without modification.
 */
...
/** Path and program name of the XDP/eBPF object loaded by the backend. */
...
/** Basic setup to connect NIC to an AF_XDP socket.
...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
    ...
    * XDP program that only redirects EtherCAT (0x88A4) frames. */
   ...
   /* Expose the XSK fd through sockhandle so the ppoll() wait path works */
...
/** Transmit buffer over the AF_XDP socket (non blocking).
 * @param[in] port        = port context struct
...
/** Non blocking read of the AF_XDP socket. Put frame in temporary buffer.
 * @param[in] port        = port context struct
 * @param[in] stacknumber = 0=primary 1=secondary stack
...
static int ecx_waitinframe_red(ecx_portt *port, uint8 idx, osal_timert *timer)
{
   ...
   do
   {
       ...
       * few microseconds, long before the next scheduler tick. ecx_inframe()
       * already performs a time-bounded busy-poll internally (via ec_xsk_recv,
       * EC_XSK_BUSYPOLL_US), so in the common case the frame is collected
       * right here without ever sleeping. This avoids the ~1 ms latency floor
       * caused by ppoll() waking up only on a timer tick. */
      if (wkc <= EC_NOFRAME)
      ...
      /* Got everything we need: leave immediately, do not sleep. */
